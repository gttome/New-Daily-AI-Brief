...
OLD = r'''      const age=(Date.parse(edition.brief_date)-Date.parse(podcast.publication_date))/86400000;
...
NEW = r'''      const published=time(podcast.published_at||podcast.publication_date);
      const observed=time(edition.research_cutoff_at||edition.brief_date);
      const ageHours=(observed-published)/3600000;
...
def main() -> int:
    ...
    if NEW in text:
        print(f'adapter already patched: {path}')
        return 0
    if OLD not in text:
        raise SystemExit(f'expected podcast freshness validator block not found: {path}')
    path.write_text(text.replace(OLD, NEW, 1), encoding='utf-8')
    print(f'patched timestamp-aware podcast freshness validation: {path}')
    return 0
