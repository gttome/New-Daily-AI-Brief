...
from new_daily_ai_brief.integration_execution_executor_binding_rehearsal import (
    IntegrationExecutionExecutorBindingRehearsalBoundaryFailure,
...
import test_iteration21


class Iteration22ExecutorBindingRehearsalTest(unittest.TestCase):
    DATE = "2026-09-22"

    def helper(self):
        return test_iteration21.Iteration21ExecutorBindingPreflightTest(
            methodName="test_current_repository_stays_blocked_deterministic_and_zero_rework"
        )

    def fixture(self, name):
        return Path(__file__).resolve().parents[1] / "fixtures" / "iteration22" / name

    def make_executor_binding_preflight(
        self, root, date=DATE, mode="synthetic", qualified=False
    ):
        h = self.helper()
        _, engine = h.make_executor_binding_readiness(root, date, mode, ready=qualified)
        kwargs = {}
        if qualified:
            kwargs["integration_execution_executor_binding_preflight_fixture_root"] = (
                h.write_fixture(Path(root) / "i21-qualified", engine)
            )
    ...
    def locked(self, engine):
        ...
        values["production-integration-execution-executor-binding-preflight"] = (
    ...
    def write_fixture(
        ...
        *,
        ...
        src = self.fixture("synthetic-executor-binding-rehearsal-complete")
        policy = json.loads((src / "executor-binding-rehearsal-policy.json").read_text())
        manifest = json.loads((src / "executor-binding-rehearsal-manifest.json").read_text())
        record = json.loads((src / "executor-binding-rehearsal-record.json").read_text())

        gate = ProductionIntegrationExecutionExecutorBindingRehearsal(
        ...
        manifest["executor_binding_preflight_binding"] = {
        ...
        record.update(
            {
                ...
                "executor_binding_preflight_id": bound[
        ...
        (root / "executor-binding-rehearsal-policy.json").write_text(json.dumps(policy))
        (root / "executor-binding-rehearsal-manifest.json").write_text(json.dumps(manifest))
        if include_record:
            (root / "executor-binding-rehearsal-record.json").write_text(
                json.dumps(record)
            )
        return root
    ...
    def test_current_repository_stays_blocked_deterministic_and_zero_rework(self):
        with tempfile.TemporaryDirectory() as td:
            counts, engine = self.make_executor_binding_preflight(td)
            ...
            self.assertEqual(data1["executor_binding_rehearsal_id"], data2["executor_binding_rehearsal_id"])
    ...
    def test_qualified_preflight_requires_separate_record_and_complete_noop_receipts(self):
        with tempfile.TemporaryDirectory() as base:
            counts, engine = self.make_executor_binding_preflight(base, qualified=True)
            locked = self.locked(engine)
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                start_daily_brief(
                    self.DATE,
                    state_root=td,
                    integration_execution_executor_binding_rehearsal_fixture_root=fixture,
                    integration_execution_executor_binding_rehearsal_only=True,
                )
                data = self.data(td)
                ...
                self.assertEqual(data["executor_binding_rehearsal_receipt_ids"], [])
                self.safe(data)
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                data = self.data(td)
                self.assertEqual(data["classification"], "executor_binding_rehearsal_complete")
                self.assertEqual(
                    data["classification_reason_codes"],
                    ["SYNTHETIC_EXECUTOR_BINDING_REHEARSAL_COMPLETE"],
                )
                self.assertEqual(
                    data["executor_binding_preflight_classification"],
                    "executor_binding_preflight_qualified",
                )
                self.assertEqual(len(data["executor_binding_rehearsal_receipt_ids"]), 10)
                self.assertEqual(len(set(data["executor_binding_rehearsal_receipt_ids"])), 10)
                for position in range(1, 11):
                    receipt = e.store.read_json(
                        e.store.run_dir
                        / f"executor-binding-rehearsal-receipt-{position:02d}.json"
                    )
    ...
    def test_fail_closed_iteration21_identity_and_provenance_substitution(self):
        with tempfile.TemporaryDirectory() as base:
            self.make_executor_binding_preflight(base, qualified=True)
            ...
            cases = [
                lambda d: d.__setitem__("executor_binding_preflight_id", "sha256:" + "0" * 64),
                lambda d: d.__setitem__("executor_binding_preflight_policy_id", "changed"),
                lambda d: d.__setitem__("executor_binding_preflight_manifest_id", "changed"),
                lambda d: d.__setitem__("separate_executor_binding_preflight_id", "sha256:" + "1" * 64),
                lambda d: d.__setitem__("synthetic_binding_plan_descriptor_id", "sha256:" + "2" * 64),
                lambda d: d.__setitem__(
                    "executor_binding_preflight_provenance_validation_ids",
                    list(reversed(d["executor_binding_preflight_provenance_validation_ids"])),
                ),
                lambda d: d.__setitem__(
                    "executor_binding_preflight_provenance_validation_ids",
                    [d["executor_binding_preflight_provenance_validation_ids"][0]] * 10,
                ),
                lambda d: d.__setitem__(
                    "executor_binding_preflight_provenance_validation_digests",
                    ["sha256:" + "3" * 64]
                    + d["executor_binding_preflight_provenance_validation_digests"][1:],
                ),
            ]
            for idx, fn in enumerate(cases):
                with self.subTest(case=idx), tempfile.TemporaryDirectory() as td:
                    ...
                    with self.assertRaises(IntegrationExecutionExecutorBindingRehearsalError):
                        start_daily_brief(
                            self.DATE,
                            state_root=td,
    ...
    def test_fail_closed_iteration22_versions_source_capabilities_authority_and_cost(self):
        with tempfile.TemporaryDirectory() as base:
            self.make_executor_binding_preflight(base, qualified=True)
            fixture_cases = {
                ...
                "policy-version": dict(
                    mutate_policy=lambda p: p.update(
                        {"executor_binding_rehearsal_policy_version": "2.0.0"}
                    )
                ),
        ...
        with tempfile.TemporaryDirectory() as base:
                ...
                with tempfile.TemporaryDirectory() as td:
                    ...
                    with self.assertRaises(
                        ...
                        start_daily_brief(
                            self.DATE,
                            state_root=td,
                            integration_execution_executor_binding_rehearsal_fixture_root=Path(td) / "fixture",
                            integration_execution_executor_binding_rehearsal_only=True,
                        )
            ...
            expect_receipt_failure(
                lambda e: mutate_final(
                    ...
                    lambda d: d.__setitem__(
                        "executor_binding_rehearsal_receipt_ids",
                        d["executor_binding_rehearsal_receipt_ids"][:-1],
                    ),
                )
            )

            def corrupt_receipt(e):
                path = e.store.run_dir / "executor-binding-rehearsal-receipt-05.json"
                r = e.store.read_json(path)
                r["source_executor_binding_preflight_validation_id"] = "sha256:" + "9" * 64
            ...
            def missing_receipt(e):
                (e.store.run_dir / "executor-binding-rehearsal-receipt-05.json").unlink()
    ...
    def recovery(self, source_root, boundary):
        ...
        failing = RunEngine(
            ...
            failure_injection=FailureInjection(
                "Complete", "synthetic_i22_failure", boundary
            ),
            integration_execution_executor_binding_rehearsal_fixture_root=fixture,
            integration_execution_executor_binding_rehearsal_only=True,
        )
        with self.assertRaises(IntegrationExecutionExecutorBindingRehearsalBoundaryFailure):
            failing.run()
        start_daily_brief(
            self.DATE,
            state_root=root,
        ...
        self.assertEqual(
            fresh.store.read_json(
                fresh.store.run_dir / "executor-binding-rehearsal-incident.json"
            )["result"],
            "recovered",
        )
        return temp, fresh
    ...
    def test_targeted_recovery_fresh_engine_no_rework(self):
        with tempfile.TemporaryDirectory() as base:
            ...
            temp, engine = self.recovery(base, "executor_binding_rehearsal:evaluation")
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-rehearsal-state.json"
            )
            self.assertEqual(state["attempts"]["evaluation"], 2)
            temp.cleanup()

            temp, engine = self.recovery(base, "executor_binding_rehearsal:receipt:5")
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-rehearsal-state.json"
            )
            ...
            temp, engine = self.recovery(
                base, "executor_binding_rehearsal:final_executor_binding_rehearsal_artifact"
            )
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-rehearsal-state.json"
            )
    ...
    def test_bounded_exclusive_production_fail_closed_and_schema(self):
        ...
        schema = json.loads(
            (
                ...
                / "schemas"
                / "production-integration-execution-executor-binding-rehearsal.schema.json"
            ).read_text()
        )
        for key in (
            "production_action_authorized",
    ...
    def test_three_run_exit_gate_is_blocked_stable_and_independent(self):
        ...
        for date in ("2026-09-22", "2026-09-23", "2026-09-24"):
            with tempfile.TemporaryDirectory() as td:
                counts, engine = self.make_executor_binding_preflight(
                    td, date, "shadow"
                )
                locked = self.locked(engine)
                run = start_daily_brief(
                ...
                outcomes.append(
                    (
                        ...
                        data["executor_binding_rehearsal_id"],
                    )
                )
