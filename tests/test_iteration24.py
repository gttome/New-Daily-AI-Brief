...
from new_daily_ai_brief.integration_execution_executor_binding_authorization_decision import (
    IntegrationExecutionExecutorBindingAuthorizationDecisionBoundaryFailure,
...
class Iteration24ExecutorBindingAuthorizationDecisionTest(unittest.TestCase):
    ...
    def emit(self, kind, data):
        destination = os.environ.get("NDAIB_ITERATION24_EVIDENCE_OUT")
        if destination:
    ...
    def snapshot(self, engine):
        return {str(p.relative_to(engine.store.run_dir)): (p.read_bytes(), p.stat().st_mtime_ns)
                ...
                and not p.name.startswith("executor-binding-authorization-decision")
                and p.name != "production-integration-execution-executor-binding-authorization-decision.json"}

    def helper(self):
        return test_iteration23.Iteration23ExecutorBindingAuthorizationReviewTest(
            methodName="test_current_repository_stays_blocked_deterministic_and_zero_rework"
        )

    def fixture(self, name):
        return Path(__file__).resolve().parents[1] / "fixtures" / "iteration24" / name

    def make_executor_binding_authorization_review(
        self, root, date=DATE, mode="synthetic", qualified=False
    ):
        h = self.helper()
        _, engine = h.make_executor_binding_rehearsal(root, date, mode, qualified=qualified)
        kwargs = {}
        if qualified:
            kwargs["integration_execution_executor_binding_authorization_review_fixture_root"] = (
                h.write_fixture(Path(root) / "i23-qualified", engine)
            )
        run = start_daily_brief(
            date,
            mode=mode,
            state_root=root,
            integration_execution_executor_binding_authorization_review_only=True,
            **kwargs,
        )
    ...
    def locked(self, engine):
        ...
        values["production-integration-execution-executor-binding-authorization-review"] = (
    ...
    def write_fixture(
        ...
        *,
        ...
        src = self.fixture("synthetic-executor-binding-authorization-decision-complete")
        policy = json.loads((src / "executor-binding-authorization-decision-policy.json").read_text())
        manifest = json.loads((src / "executor-binding-authorization-decision-manifest.json").read_text())
        record = json.loads((src / "executor-binding-authorization-decision-record.json").read_text())

        gate = ProductionIntegrationExecutionExecutorBindingAuthorizationDecision(
        ...
        manifest["executor_binding_authorization_review_binding"] = {
        ...
        record["executor_binding_authorization_review_evidence_ids"] = bound["executor_binding_authorization_review_evidence_ids"]
        record["executor_binding_authorization_review_evidence_digests"] = bound["executor_binding_authorization_review_evidence_digests"]
        record.update(
            {
                "executor_binding_authorization_review_artifact_digest": bound[
                    "executor_binding_authorization_review_artifact_digest"
                ],
                "executor_binding_authorization_review_id": bound[
        ...
        (root / "executor-binding-authorization-decision-policy.json").write_text(json.dumps(policy))
        (root / "executor-binding-authorization-decision-manifest.json").write_text(json.dumps(manifest))
        if include_record:
            (root / "executor-binding-authorization-decision-record.json").write_text(
                json.dumps(record)
            )
        return root
    ...
    def test_current_repository_stays_blocked_deterministic_and_zero_rework(self):
        with tempfile.TemporaryDirectory() as td:
            counts, engine = self.make_executor_binding_authorization_review(td)
            ...
            self.assertEqual(data1["executor_binding_authorization_decision_id"], data2["executor_binding_authorization_decision_id"])
    ...
    def test_complete_review_requires_separate_record_and_complete_decision_evidence(self):
        with tempfile.TemporaryDirectory() as base:
            counts, engine = self.make_executor_binding_authorization_review(base, qualified=True)
            locked = self.locked(engine)
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                start_daily_brief(
                    self.DATE,
                    state_root=td,
                    integration_execution_executor_binding_authorization_decision_fixture_root=fixture,
                    integration_execution_executor_binding_authorization_decision_only=True,
                )
                data = self.data(td)
                ...
                self.assertEqual(data["executor_binding_authorization_decision_evidence_ids"], [])
                self.safe(data)
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                data = self.data(td)
                self.assertEqual(data["classification"], "executor_binding_authorization_decision_complete")
                self.assertEqual(
                    data["classification_reason_codes"],
                    ["SYNTHETIC_EXECUTOR_BINDING_AUTHORIZATION_DECISION_COMPLETE"],
                )
                self.assertEqual(
                    data["executor_binding_authorization_review_classification"],
                    "executor_binding_authorization_review_complete",
                )
                schema = json.loads((Path(__file__).resolve().parents[1] / "schemas" /
                    "production-integration-execution-executor-binding-authorization-decision.schema.json").read_text())
                ...
                self.assertEqual(len(set(data["executor_binding_authorization_decision_evidence_ids"])), 10)
                for position in range(1, 11):
                    receipt = e.store.read_json(
                        e.store.run_dir
                        / f"executor-binding-authorization-decision-evidence-{position:02d}.json"
                    )
    ...
    def test_fail_closed_iteration23_identity_and_provenance_substitution(self):
        with tempfile.TemporaryDirectory() as base:
            self.make_executor_binding_authorization_review(base, qualified=True)
            ...
            cases = [
                lambda d: d.__setitem__("executor_binding_authorization_review_id", "sha256:" + "0" * 64),
                lambda d: d.__setitem__("executor_binding_authorization_review_policy_id", "changed"),
                lambda d: d.__setitem__("executor_binding_authorization_review_manifest_id", "changed"),
                lambda d: d.__setitem__("separate_executor_binding_authorization_review_id", "sha256:" + "1" * 64),
                lambda d: d.__setitem__("synthetic_binding_plan_descriptor_id", "sha256:" + "2" * 64),
                lambda d: d.__setitem__(
                    "executor_binding_authorization_review_evidence_ids",
                    list(reversed(d["executor_binding_authorization_review_evidence_ids"])),
                ),
                lambda d: d.__setitem__(
                    "executor_binding_authorization_review_evidence_ids",
                    [d["executor_binding_authorization_review_evidence_ids"][0]] * 10,
                ),
                lambda d: d.__setitem__(
                    "executor_binding_authorization_review_evidence_digests",
                    ["sha256:" + "3" * 64]
                    + d["executor_binding_authorization_review_evidence_digests"][1:],
                ),
            ]
            for idx, fn in enumerate(cases):
                with self.subTest(case=idx), tempfile.TemporaryDirectory() as td:
                    ...
                    with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationDecisionError):
                        start_daily_brief(
                            self.DATE,
                            state_root=td,
            ...
            fixture_cases = {
                ...
                "policy-version": dict(
                    mutate_policy=lambda p: p.update(
                        {"executor_binding_authorization_decision_policy_version": "2.0.0"}
                    )
                ),
        ...
        with tempfile.TemporaryDirectory() as base:
                ...
                with tempfile.TemporaryDirectory() as td:
                    ...
                    with self.assertRaises(
                        ...
                        start_daily_brief(
                            self.DATE,
                            state_root=td,
                            integration_execution_executor_binding_authorization_decision_fixture_root=Path(td) / "fixture",
                            integration_execution_executor_binding_authorization_decision_only=True,
                        )
            ...
            expect_receipt_failure(
                lambda e: mutate_final(
                    ...
                    lambda d: d.__setitem__(
                        "executor_binding_authorization_decision_evidence_ids",
                        d["executor_binding_authorization_decision_evidence_ids"][:-1],
                    ),
                )
            )

            def corrupt_receipt(e):
                path = e.store.run_dir / "executor-binding-authorization-decision-evidence-05.json"
                r = e.store.read_json(path)
                r["source_executor_binding_authorization_review_evidence_id"] = "sha256:" + "9" * 64
            ...
            def missing_receipt(e):
                (e.store.run_dir / "executor-binding-authorization-decision-evidence-05.json").unlink()
        ...
        with tempfile.TemporaryDirectory() as base:
            ...
            for case in ("reordered", "duplicated", "missing", "substituted", "corrupted", "version", "upstream-stale"):
                with self.subTest(case=case), tempfile.TemporaryDirectory() as td:
                    self.clone_root(base, td)
                    e = RunEngine(td, self.DATE)
                    p = e.store.run_dir / "executor-binding-authorization-review-evidence-05.json"
                    q = e.store.run_dir / "executor-binding-authorization-review-evidence-06.json"
                    if case == "missing":
                        p.unlink()
                    elif case == "reordered":
        ...
        with tempfile.TemporaryDirectory() as base:
            ...
            for case in ("cache", "artifact-version", "artifact-id", "record-id", "manifest-version"):
                with self.subTest(case=case), tempfile.TemporaryDirectory() as td:
                    ...
                    e = RunEngine(td, self.DATE)
                    if case == "cache":
                        p = e.store.run_dir / "executor-binding-authorization-decision-state.json"
                        a = e.store.read_json(p)
                        a["evaluation"]["classification"] = "blocked"
                    elif case.startswith("artifact"):
                        p = e.store.artifact_path("production-integration-execution-executor-binding-authorization-decision")
                        a = e.store.read_json(p)
                        k = "executor_binding_authorization_decision_" + ("schema_version" if case.endswith("version") else "id")
                        a["data"][k] = "changed"
                        a["content_digest"] = semantic_digest(a)
                    elif case == "record-id":
                        p = Path(td) / "fixture" / "executor-binding-authorization-decision-record.json"
                        a = e.store.read_json(p)
                        a["authorization_decision_record_id"] = "changed"
                    else:
                        p = Path(td) / "fixture" / "executor-binding-authorization-decision-manifest.json"
                        a = e.store.read_json(p)
                        a["executor_binding_authorization_decision_manifest_version"] = "9.0.0"
                    e.store._atomic_write(p, a)
                    with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationDecisionError):
                        start_daily_brief(self.DATE, state_root=td,
                            integration_execution_executor_binding_authorization_decision_fixture_root=Path(td) / "fixture",
    ...
    def recovery(self, source_root, boundary):
        ...
        with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationDecisionBoundaryFailure):
            failing.run()
        durable = {p.name: (p.read_bytes(), p.stat().st_mtime_ns)
                   for p in engine.store.run_dir.glob("executor-binding-authorization-decision-evidence-*.json")}
        start_daily_brief(
            self.DATE,
            state_root=root,
            integration_execution_executor_binding_authorization_decision_fixture_root=fixture,
        ...
        self.assertEqual(
            fresh.store.read_json(
                fresh.store.run_dir / "executor-binding-authorization-decision-incident.json"
            )["result"],
            "recovered",
        )
        ...
        state = fresh.store.read_json(fresh.store.run_dir / "executor-binding-authorization-decision-state.json")
        self.emit("recovery", {"boundary": boundary, "attempts": state["attempts"],
                  ...
                  "iteration23_rebuilds": 0, "unrelated_record_rewrites": 0,
    ...
    def test_targeted_recovery_fresh_engine_no_rework(self):
        with tempfile.TemporaryDirectory() as base:
            ...
            temp, engine = self.recovery(base, "executor_binding_authorization_decision:evaluation")
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-authorization-decision-state.json"
            )
            self.assertEqual(state["attempts"]["evaluation"], 2)
            temp.cleanup()

            temp, engine = self.recovery(base, "executor_binding_authorization_decision:receipt:5")
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-authorization-decision-state.json"
            )
            ...
            temp, engine = self.recovery(
                base, "executor_binding_authorization_decision:final_executor_binding_authorization_decision_artifact"
            )
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-authorization-decision-state.json"
            )
    ...
    def test_bounded_exclusive_production_fail_closed_and_schema(self):
        ...
        schema = json.loads(
            (
                ...
                / "schemas"
                / "production-integration-execution-executor-binding-authorization-decision.schema.json"
            ).read_text()
        )
        for key in (
            "production_action_authorized",
    ...
    def test_three_run_exit_gate_is_blocked_stable_and_independent(self):
        ...
        for date in ("2026-09-22", "2026-09-23", "2026-09-24"):
            with tempfile.TemporaryDirectory() as td:
                counts, engine = self.make_executor_binding_authorization_review(
                    td, date, "shadow"
                )
                locked = self.locked(engine)
                prior_files = self.snapshot(engine)
                ...
                self.assertEqual(data["classification"], "blocked")
                self.assertEqual(data["classification_reason_codes"][0], "ITERATION23_EXECUTOR_BINDING_AUTHORIZATION_REVIEW_BLOCKED")
                artifact = engine.store.load_artifact("production-integration-execution-executor-binding-authorization-decision")
                self.emit("exit_run", {"date": date, "mode": "shadow", "artifact_digest": artifact["content_digest"],
                          "decision_id": data["executor_binding_authorization_decision_id"],
                          "source_digest": data["executor_binding_authorization_review_artifact_digest"],
                          "classification": data["classification"], "reason_codes": data["classification_reason_codes"],
                          "locked_upstream_artifact_count": len(locked), "upstream_reexecution": 0,
                          "iteration23_rebuilds": 0, "unrelated_record_rewrites": 0, "full_pipeline_restarts": 0,
                ...
                outcomes.append(
                    (
                        ...
                        data["executor_binding_authorization_decision_id"],
                    )
                )
    ...
    def test_decision_record_exact_inventory_and_all_bound_identities(self):
        with tempfile.TemporaryDirectory() as base:
            ...
            original = json.loads((fixture / "executor-binding-authorization-decision-record.json").read_text())
            ...
            for field in ("executor_binding_authorization_review_evidence_ids",
                          "executor_binding_authorization_review_evidence_digests"):
            ...
            for name, value in changes.items():
                with self.subTest(name=name), tempfile.TemporaryDirectory() as td:
                    self.clone_root(base, td)
                    path = Path(td) / "fixture" / "executor-binding-authorization-decision-record.json"
                    ...
                    with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationDecisionError):
                        start_daily_brief(self.DATE, state_root=td,
                            integration_execution_executor_binding_authorization_decision_only=True,
                            integration_execution_executor_binding_authorization_decision_fixture_root=path.parent)
    ...
    def test_unsafe_unknown_fields_and_all_mutations_rejected_on_blocked_source(self):
        with tempfile.TemporaryDirectory() as base:
            self.make_executor_binding_authorization_review(base)
    ...
    def test_all_bounded_modes_exclusive_and_prior_gate_never_prepared_or_built(self):
        ...
        from new_daily_ai_brief.integration_execution_executor_binding_authorization_review import ProductionIntegrationExecutionExecutorBindingAuthorizationReview as Prior
        option = "integration_execution_executor_binding_authorization_decision_only"
        ...
        with tempfile.TemporaryDirectory() as td:
            self.make_executor_binding_authorization_review(td)
            with patch.object(Prior, "prepare", side_effect=AssertionError("prior evaluation forbidden")), patch.object(Prior, "build_executor_binding_authorization_review", side_effect=AssertionError("prior rebuild forbidden")):
