...
from new_daily_ai_brief.integration_execution_executor_binding_authorization_package_readiness_authorization_review import (
    IntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationReviewBoundaryFailure,
...
import test_iteration28


class Iteration29ExecutorBindingAuthorizationPackageReadinessAuthorizationReviewTest(unittest.TestCase):
    DATE = "2026-09-22"

    def emit(self, kind, data):
        destination = os.environ.get("NDAIB_ITERATION29_EVIDENCE_OUT")
        if destination:
    ...
    def snapshot(self, engine):
        return {str(p.relative_to(engine.store.run_dir)): (p.read_bytes(), p.stat().st_mtime_ns)
                ...
                and not p.name.startswith("executor-binding-authorization-package-readiness-authorization-review")
                and p.name != "production-integration-execution-executor-binding-authorization-package-readiness-authorization-review.json"}

    def helper(self):
        return test_iteration28.Iteration28ExecutorBindingAuthorizationPackageReadinessRehearsalTest(
            methodName="test_current_repository_stays_blocked_deterministic_and_zero_rework"
        )

    def fixture(self, name):
        return Path(__file__).resolve().parents[1] / "fixtures" / "iteration29" / name

    def make_executor_binding_authorization_package_readiness_rehearsal(
        self, root, date=DATE, mode="synthetic", qualified=False
    ):
        h = self.helper()
        _, engine = h.make_executor_binding_authorization_package_readiness_preflight(root, date, mode, qualified=qualified)
        kwargs = {}
        if qualified:
            kwargs["integration_execution_executor_binding_authorization_package_readiness_rehearsal_fixture_root"] = (
                h.write_fixture(Path(root) / "i28-qualified", engine)
            )
    ...
    def locked(self, engine):
        ...
        values["production-integration-execution-executor-binding-authorization-package-readiness-rehearsal"] = (
    ...
    def write_fixture(
        ...
        *,
        ...
        src = self.fixture("synthetic-executor-binding-authorization-package-readiness-authorization-review-complete")
        policy = json.loads((src / "executor-binding-authorization-package-readiness-authorization-review-policy.json").read_text())
        manifest = json.loads((src / "executor-binding-authorization-package-readiness-authorization-review-manifest.json").read_text())
        record = json.loads((src / "executor-binding-authorization-package-readiness-authorization-review-record.json").read_text())

        gate = ProductionIntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationReview(
        ...
        manifest["executor_binding_authorization_package_readiness_rehearsal_binding"] = {
        ...
        record.update(
            {
                ...
                "executor_binding_authorization_package_readiness_rehearsal_id": bound[
        ...
        (root / "executor-binding-authorization-package-readiness-authorization-review-policy.json").write_text(json.dumps(policy))
        (root / "executor-binding-authorization-package-readiness-authorization-review-manifest.json").write_text(json.dumps(manifest))
        if include_record:
            (root / "executor-binding-authorization-package-readiness-authorization-review-record.json").write_text(
                json.dumps(record)
            )
        return root
    ...
    def test_current_repository_stays_blocked_deterministic_and_zero_rework(self):
        with tempfile.TemporaryDirectory() as td:
            counts, engine = self.make_executor_binding_authorization_package_readiness_rehearsal(td)
    ...
    def test_complete_rehearsal_requires_separate_record_and_complete_review_evidence(self):
        with tempfile.TemporaryDirectory() as base:
            counts, engine = self.make_executor_binding_authorization_package_readiness_rehearsal(base, qualified=True)
            locked = self.locked(engine)
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                start_daily_brief(
                    self.DATE,
                    state_root=td,
                    integration_execution_executor_binding_authorization_package_readiness_authorization_review_fixture_root=fixture,
                    integration_execution_executor_binding_authorization_package_readiness_authorization_review_only=True,
                )
                data = self.data(td)
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                schema = json.loads((Path(__file__).resolve().parents[1] / "schemas" /
                    "production-integration-execution-executor-binding-authorization-package-readiness-authorization-review.schema.json").read_text())
                for field in schema["required"] + schema["allOf"][0]["then"]["required"]:
                    self.assertIn(field, data)
                self.assertEqual(len(data["executor_binding_authorization_package_readiness_authorization_review_evidence_ids"]), 10)
                self.assertEqual(len(set(data["executor_binding_authorization_package_readiness_authorization_review_evidence_ids"])), 10)
                for position in range(1, 11):
                    receipt = e.store.read_json(
                        e.store.run_dir
                        / f"executor-binding-authorization-package-readiness-authorization-review-evidence-{position:02d}.json"
                    )
    ...
    def test_fail_closed_iteration28_identity_and_provenance_substitution(self):
        with tempfile.TemporaryDirectory() as base:
            self.make_executor_binding_authorization_package_readiness_rehearsal(base, qualified=True)
            ...
            cases = [
                lambda d: d.__setitem__("executor_binding_authorization_package_readiness_rehearsal_id", "sha256:" + "0" * 64),
                lambda d: d.__setitem__("executor_binding_authorization_package_readiness_rehearsal_policy_id", "changed"),
                ...
                lambda d: d.__setitem__(
                    "executor_binding_authorization_package_readiness_rehearsal_receipt_digests",
                    ["sha256:" + "3" * 64]
                    + d["executor_binding_authorization_package_readiness_rehearsal_receipt_digests"][1:],
                ),
            ]
            for idx, fn in enumerate(cases):
                with self.subTest(case=idx), tempfile.TemporaryDirectory() as td:
                    ...
                    with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationReviewError):
                        start_daily_brief(
                            self.DATE,
                            state_root=td,
    ...
    def test_fail_closed_iteration29_versions_source_capabilities_authority_and_cost(self):
        with tempfile.TemporaryDirectory() as base:
            self.make_executor_binding_authorization_package_readiness_rehearsal(base, qualified=True)
            fixture_cases = {
                ...
                "policy-version": dict(
                    mutate_policy=lambda p: p.update(
                        {"executor_binding_authorization_package_readiness_authorization_review_policy_version": "2.0.0"}
                    )
                ),
        ...
        with tempfile.TemporaryDirectory() as base:
                ...
                with tempfile.TemporaryDirectory() as td:
                    ...
                    with self.assertRaises(
                        ...
                        start_daily_brief(
                            self.DATE,
                            state_root=td,
                            integration_execution_executor_binding_authorization_package_readiness_authorization_review_fixture_root=Path(td) / "fixture",
                            integration_execution_executor_binding_authorization_package_readiness_authorization_review_only=True,
                        )
            ...
            expect_receipt_failure(
                lambda e: mutate_final(
                    ...
                    lambda d: d.__setitem__(
                        "executor_binding_authorization_package_readiness_authorization_review_evidence_ids",
                        d["executor_binding_authorization_package_readiness_authorization_review_evidence_ids"][:-1],
                    ),
                )
            )

            def corrupt_receipt(e):
                path = e.store.run_dir / "executor-binding-authorization-package-readiness-authorization-review-evidence-05.json"
                r = e.store.read_json(path)
                r["source_executor_binding_authorization_package_readiness_rehearsal_receipt_id"] = "sha256:" + "9" * 64
            ...
            def missing_receipt(e):
                (e.store.run_dir / "executor-binding-authorization-package-readiness-authorization-review-evidence-05.json").unlink()
        ...
        with tempfile.TemporaryDirectory() as base:
            ...
            for case in ("cache", "artifact-version", "artifact-id", "record-id", "manifest-version"):
                with self.subTest(case=case), tempfile.TemporaryDirectory() as td:
                    ...
                    e = RunEngine(td, self.DATE)
                    if case == "cache":
                        p = e.store.run_dir / "executor-binding-authorization-package-readiness-authorization-review-state.json"
                        a = e.store.read_json(p)
                        a["evaluation"]["classification"] = "blocked"
                    elif case.startswith("artifact"):
                        p = e.store.artifact_path("production-integration-execution-executor-binding-authorization-package-readiness-authorization-review")
                        a = e.store.read_json(p)
                        k = "executor_binding_authorization_package_readiness_authorization_review_" + ("schema_version" if case.endswith("version") else "id")
                        a["data"][k] = "changed"
                        a["content_digest"] = semantic_digest(a)
                    elif case == "record-id":
                        p = Path(td) / "fixture" / "executor-binding-authorization-package-readiness-authorization-review-record.json"
                        a = e.store.read_json(p)
                        a["authorization_review_record_id"] = "changed"
                    else:
                        p = Path(td) / "fixture" / "executor-binding-authorization-package-readiness-authorization-review-manifest.json"
                        a = e.store.read_json(p)
                        a["executor_binding_authorization_package_readiness_authorization_review_manifest_version"] = "9.0.0"
                    e.store._atomic_write(p, a)
                    with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationReviewError):
                        start_daily_brief(self.DATE, state_root=td,
                            integration_execution_executor_binding_authorization_package_readiness_authorization_review_fixture_root=Path(td) / "fixture",
    ...
    def recovery(self, source_root, boundary):
        ...
        failing = RunEngine(
            ...
            failure_injection=FailureInjection(
                "Complete", "synthetic_i29_failure", boundary
            ),
            integration_execution_executor_binding_authorization_package_readiness_authorization_review_fixture_root=fixture,
            integration_execution_executor_binding_authorization_package_readiness_authorization_review_only=True,
        )
        with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationReviewBoundaryFailure):
            failing.run()
        durable = {p.name: (p.read_bytes(), p.stat().st_mtime_ns)
                   for p in engine.store.run_dir.glob("executor-binding-authorization-package-readiness-authorization-review-evidence-*.json")}
        start_daily_brief(
            self.DATE,
            state_root=root,
            integration_execution_executor_binding_authorization_package_readiness_authorization_review_fixture_root=fixture,
        ...
        self.assertEqual(
            fresh.store.read_json(
                fresh.store.run_dir / "executor-binding-authorization-package-readiness-authorization-review-incident.json"
            )["result"],
            "recovered",
        )
        ...
        state = fresh.store.read_json(fresh.store.run_dir / "executor-binding-authorization-package-readiness-authorization-review-state.json")
        self.emit("recovery", {"boundary": boundary, "attempts": state["attempts"],
                  ...
                  "iteration28_rebuilds": 0, "unrelated_record_rewrites": 0,
    ...
    def test_targeted_recovery_fresh_engine_no_rework(self):
        with tempfile.TemporaryDirectory() as base:
            ...
            temp, engine = self.recovery(base, "executor_binding_authorization_package_readiness_authorization_review:evaluation")
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-authorization-package-readiness-authorization-review-state.json"
            )
            self.assertEqual(state["attempts"]["evaluation"], 2)
            temp.cleanup()

            temp, engine = self.recovery(base, "executor_binding_authorization_package_readiness_authorization_review:receipt:5")
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-authorization-package-readiness-authorization-review-state.json"
            )
            ...
            temp, engine = self.recovery(
                base, "executor_binding_authorization_package_readiness_authorization_review:final_executor_binding_authorization_package_readiness_authorization_review_artifact"
            )
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-authorization-package-readiness-authorization-review-state.json"
            )
    ...
    def test_bounded_exclusive_production_fail_closed_and_schema(self):
        ...
        schema = json.loads(
            (
                ...
                / "schemas"
                / "production-integration-execution-executor-binding-authorization-package-readiness-authorization-review.schema.json"
            ).read_text()
        )
        for key in (
            "production_action_authorized",
    ...
    def test_three_run_exit_gate_is_blocked_stable_and_independent(self):
        ...
        for date in ("2026-09-22", "2026-09-23", "2026-09-24"):
            with tempfile.TemporaryDirectory() as td:
                counts, engine = self.make_executor_binding_authorization_package_readiness_rehearsal(
                    td, date, "shadow"
                )
                locked = self.locked(engine)
                prior_files = self.snapshot(engine)
                ...
                self.emit("exit_run", {"date": date, "mode": "shadow", "artifact_digest": artifact["content_digest"],
                          ...
                          "source_digest": data["executor_binding_authorization_package_readiness_rehearsal_artifact_digest"],
                          "classification": data["classification"], "reason_codes": data["classification_reason_codes"],
                          "locked_upstream_artifact_count": len(locked), "upstream_reexecution": 0,
                          "iteration28_rebuilds": 0, "unrelated_record_rewrites": 0, "full_pipeline_restarts": 0,
                ...
                outcomes.append(
                    (
                        ...
                        data["executor_binding_authorization_package_readiness_authorization_review_id"],
                    )
                )
