...
from new_daily_ai_brief.integration_execution_executor_binding_authorization_review import (
    IntegrationExecutionExecutorBindingAuthorizationReviewBoundaryFailure,
...
import test_iteration22


class Iteration23ExecutorBindingAuthorizationReviewTest(unittest.TestCase):
    DATE = "2026-09-22"

    def emit(self, kind, data):
        destination = os.environ.get("NDAIB_ITERATION23_EVIDENCE_OUT")
        if destination:
    ...
    def fixture(self, name):
        return Path(__file__).resolve().parents[1] / "fixtures" / "iteration23" / name

    def make_executor_binding_rehearsal(
        self, root, date=DATE, mode="synthetic", qualified=False
    ):
        h = self.helper()
        _, engine = h.make_executor_binding_preflight(root, date, mode, qualified=qualified)
        kwargs = {}
        if qualified:
            kwargs["integration_execution_executor_binding_rehearsal_fixture_root"] = (
                h.write_fixture(Path(root) / "i22-qualified", engine)
            )
    ...
    def locked(self, engine):
        ...
        values["production-integration-execution-executor-binding-rehearsal"] = (
    ...
    def write_fixture(
        ...
        *,
        ...
        src = self.fixture("synthetic-executor-binding-authorization-review-complete")
        policy = json.loads((src / "executor-binding-authorization-review-policy.json").read_text())
        manifest = json.loads((src / "executor-binding-authorization-review-manifest.json").read_text())
        record = json.loads((src / "executor-binding-authorization-review-record.json").read_text())

        gate = ProductionIntegrationExecutionExecutorBindingAuthorizationReview(
        ...
        manifest["executor_binding_rehearsal_binding"] = {
        ...
        record.update(
            {
                ...
                "executor_binding_rehearsal_id": bound[
        ...
        body.pop("authorization_review_record_id", None)
        record["authorization_review_record_id"] = digest(body)

        (root / "executor-binding-authorization-review-policy.json").write_text(json.dumps(policy))
        (root / "executor-binding-authorization-review-manifest.json").write_text(json.dumps(manifest))
        if include_record:
            (root / "executor-binding-authorization-review-record.json").write_text(
                json.dumps(record)
            )
        return root
    ...
    def test_current_repository_stays_blocked_deterministic_and_zero_rework(self):
        with tempfile.TemporaryDirectory() as td:
            counts, engine = self.make_executor_binding_rehearsal(td)
            ...
            self.assertEqual(data1["executor_binding_authorization_review_id"], data2["executor_binding_authorization_review_id"])
    ...
    def test_complete_rehearsal_requires_separate_record_and_complete_review_evidence(self):
        with tempfile.TemporaryDirectory() as base:
            counts, engine = self.make_executor_binding_rehearsal(base, qualified=True)
            locked = self.locked(engine)
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                start_daily_brief(
                    self.DATE,
                    state_root=td,
                    integration_execution_executor_binding_authorization_review_fixture_root=fixture,
                    integration_execution_executor_binding_authorization_review_only=True,
                )
                data = self.data(td)
                ...
                self.assertEqual(data["executor_binding_authorization_review_evidence_ids"], [])
                self.safe(data)
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                data = self.data(td)
                self.assertEqual(data["classification"], "executor_binding_authorization_review_complete")
                self.assertEqual(
                    data["classification_reason_codes"],
                    ["SYNTHETIC_EXECUTOR_BINDING_AUTHORIZATION_REVIEW_COMPLETE"],
                )
                self.assertEqual(
                    data["executor_binding_rehearsal_classification"],
                    "executor_binding_rehearsal_complete",
                )
                schema = json.loads((Path(__file__).resolve().parents[1] / "schemas" /
                    "production-integration-execution-executor-binding-authorization-review.schema.json").read_text())
                for field in schema["required"] + schema["allOf"][0]["then"]["required"]:
                    self.assertIn(field, data)
                self.assertEqual(len(data["executor_binding_authorization_review_evidence_ids"]), 10)
                self.assertEqual(len(set(data["executor_binding_authorization_review_evidence_ids"])), 10)
                for position in range(1, 11):
                    receipt = e.store.read_json(
                        e.store.run_dir
                        / f"executor-binding-authorization-review-evidence-{position:02d}.json"
                    )
    ...
    def test_fail_closed_iteration22_identity_and_provenance_substitution(self):
        with tempfile.TemporaryDirectory() as base:
            self.make_executor_binding_rehearsal(base, qualified=True)
            ...
            cases = [
                lambda d: d.__setitem__("executor_binding_rehearsal_id", "sha256:" + "0" * 64),
                lambda d: d.__setitem__("executor_binding_rehearsal_policy_id", "changed"),
                lambda d: d.__setitem__("executor_binding_rehearsal_manifest_id", "changed"),
                lambda d: d.__setitem__("separate_executor_binding_rehearsal_id", "sha256:" + "1" * 64),
                lambda d: d.__setitem__("synthetic_binding_plan_descriptor_id", "sha256:" + "2" * 64),
                lambda d: d.__setitem__(
                    "executor_binding_rehearsal_receipt_ids",
                    list(reversed(d["executor_binding_rehearsal_receipt_ids"])),
                ),
                lambda d: d.__setitem__(
                    "executor_binding_rehearsal_receipt_ids",
                    [d["executor_binding_rehearsal_receipt_ids"][0]] * 10,
                ),
                lambda d: d.__setitem__(
                    "executor_binding_rehearsal_receipt_digests",
                    ["sha256:" + "3" * 64]
                    + d["executor_binding_rehearsal_receipt_digests"][1:],
                ),
            ]
            for idx, fn in enumerate(cases):
                with self.subTest(case=idx), tempfile.TemporaryDirectory() as td:
                    ...
                    with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationReviewError):
                        start_daily_brief(
                            self.DATE,
                            state_root=td,
    ...
    def test_fail_closed_iteration23_versions_source_capabilities_authority_and_cost(self):
        with tempfile.TemporaryDirectory() as base:
            self.make_executor_binding_rehearsal(base, qualified=True)
            fixture_cases = {
                ...
                "policy-version": dict(
                    mutate_policy=lambda p: p.update(
                        {"executor_binding_authorization_review_policy_version": "2.0.0"}
                    )
                ),
        ...
        with tempfile.TemporaryDirectory() as base:
                ...
                with tempfile.TemporaryDirectory() as td:
                    ...
                    with self.assertRaises(
                        ...
                        start_daily_brief(
                            self.DATE,
                            state_root=td,
                            integration_execution_executor_binding_authorization_review_fixture_root=Path(td) / "fixture",
                            integration_execution_executor_binding_authorization_review_only=True,
                        )
            ...
            expect_receipt_failure(
                lambda e: mutate_final(
                    ...
                    lambda d: d.__setitem__(
                        "executor_binding_authorization_review_evidence_ids",
                        d["executor_binding_authorization_review_evidence_ids"][:-1],
                    ),
                )
            )

            def corrupt_receipt(e):
                path = e.store.run_dir / "executor-binding-authorization-review-evidence-05.json"
                r = e.store.read_json(path)
                r["source_executor_binding_rehearsal_receipt_id"] = "sha256:" + "9" * 64
            ...
            def missing_receipt(e):
                (e.store.run_dir / "executor-binding-authorization-review-evidence-05.json").unlink()
    ...
    def test_changed_review_cache_and_identity_fail_closed(self):
        with tempfile.TemporaryDirectory() as base:
            self.make_executor_binding_rehearsal(base, qualified=True)
            engine = RunEngine(base, self.DATE)
            fixture = self.write_fixture(Path(base) / "fixture", engine)
            ...
            for case in ("cache", "artifact-version", "artifact-id", "record-id", "manifest-version"):
                with self.subTest(case=case), tempfile.TemporaryDirectory() as td:
                    ...
                    e = RunEngine(td, self.DATE)
                    if case == "cache":
                        p = e.store.run_dir / "executor-binding-authorization-review-state.json"
                        a = e.store.read_json(p)
                        a["evaluation"]["classification"] = "blocked"
                    elif case.startswith("artifact"):
                        p = e.store.artifact_path("production-integration-execution-executor-binding-authorization-review")
                        a = e.store.read_json(p)
                        k = "executor_binding_authorization_review_" + ("schema_version" if case.endswith("version") else "id")
                        a["data"][k] = "changed"
                        a["content_digest"] = semantic_digest(a)
                    elif case == "record-id":
                        p = Path(td) / "fixture" / "executor-binding-authorization-review-record.json"
                        a = e.store.read_json(p)
                        a["authorization_review_record_id"] = "changed"
                    else:
                        p = Path(td) / "fixture" / "executor-binding-authorization-review-manifest.json"
                        a = e.store.read_json(p)
                        a["executor_binding_authorization_review_manifest_version"] = "9.0.0"
                    e.store._atomic_write(p, a)
                    with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationReviewError):
                        start_daily_brief(self.DATE, state_root=td,
                            integration_execution_executor_binding_authorization_review_fixture_root=Path(td) / "fixture",
    ...
    def recovery(self, source_root, boundary):
        ...
        failing = RunEngine(
            ...
            failure_injection=FailureInjection(
                "Complete", "synthetic_i23_failure", boundary
            ),
            integration_execution_executor_binding_authorization_review_fixture_root=fixture,
            integration_execution_executor_binding_authorization_review_only=True,
        )
        with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationReviewBoundaryFailure):
            failing.run()
        durable = {p.name: (p.read_bytes(), p.stat().st_mtime_ns)
                   for p in engine.store.run_dir.glob("executor-binding-authorization-review-evidence-*.json")}
        start_daily_brief(
            self.DATE,
            state_root=root,
            integration_execution_executor_binding_authorization_review_fixture_root=fixture,
        ...
        self.assertEqual(
            fresh.store.read_json(
                fresh.store.run_dir / "executor-binding-authorization-review-incident.json"
            )["result"],
            "recovered",
        )
        ...
        state = fresh.store.read_json(fresh.store.run_dir / "executor-binding-authorization-review-state.json")
        self.emit("recovery", {"boundary": boundary, "attempts": state["attempts"],
                  "metrics": state["metrics"], "prior_durable_review_evidence": len(durable),
                  "locked_upstream_artifact_count": len(locked), "upstream_reexecution": 0,
                  "iteration22_rebuilds": 0, "unrelated_record_rewrites": 0,
    ...
    def test_targeted_recovery_fresh_engine_no_rework(self):
        with tempfile.TemporaryDirectory() as base:
            ...
            temp, engine = self.recovery(base, "executor_binding_authorization_review:evaluation")
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-authorization-review-state.json"
            )
            self.assertEqual(state["attempts"]["evaluation"], 2)
            temp.cleanup()

            temp, engine = self.recovery(base, "executor_binding_authorization_review:receipt:5")
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-authorization-review-state.json"
            )
            ...
            temp, engine = self.recovery(
                base, "executor_binding_authorization_review:final_executor_binding_authorization_review_artifact"
            )
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-authorization-review-state.json"
            )
    ...
    def test_bounded_exclusive_production_fail_closed_and_schema(self):
        ...
        schema = json.loads(
            (
                ...
                / "schemas"
                / "production-integration-execution-executor-binding-authorization-review.schema.json"
            ).read_text()
        )
        for key in (
            "production_action_authorized",
    ...
    def test_three_run_exit_gate_is_blocked_stable_and_independent(self):
        ...
        for date in ("2026-09-22", "2026-09-23", "2026-09-24"):
            with tempfile.TemporaryDirectory() as td:
                counts, engine = self.make_executor_binding_rehearsal(
                    td, date, "shadow"
                )
                locked = self.locked(engine)
                prior_files = self.snapshot(engine)
                ...
                artifact = engine.store.load_artifact("production-integration-execution-executor-binding-authorization-review")
                self.emit("exit_run", {"date": date, "mode": "shadow", "artifact_digest": artifact["content_digest"],
                          "review_id": data["executor_binding_authorization_review_id"],
                          "source_digest": data["executor_binding_rehearsal_artifact_digest"],
                          "classification": data["classification"], "reason_codes": data["classification_reason_codes"],
                          "locked_upstream_artifact_count": len(locked), "upstream_reexecution": 0,
                          "iteration22_rebuilds": 0, "unrelated_record_rewrites": 0, "full_pipeline_restarts": 0,
                ...
                outcomes.append(
                    (
                        ...
                        data["executor_binding_authorization_review_id"],
                    )
                )
