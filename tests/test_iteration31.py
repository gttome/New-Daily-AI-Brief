...
from new_daily_ai_brief.integration_execution_executor_binding_authorization_package_readiness_authorization_package import (
    IntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationPackageBoundaryFailure,
    IntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationPackageError,
    ProductionIntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationPackage,
)
from new_daily_ai_brief.store import digest, semantic_digest
import test_iteration30


class Iteration31ExecutorBindingAuthorizationPackageReadinessAuthorizationPackageTest(unittest.TestCase):
    DATE = "2026-09-22"

    def emit(self, kind, data):
        destination = os.environ.get("NDAIB_ITERATION31_EVIDENCE_OUT")
        if destination:
    ...
    def snapshot(self, engine):
        return {str(p.relative_to(engine.store.run_dir)): (p.read_bytes(), p.stat().st_mtime_ns)
                ...
                and not p.name.startswith("executor-binding-authorization-package-readiness-authorization-package")
                and p.name != "production-integration-execution-executor-binding-authorization-package-readiness-authorization-package.json"}

    def helper(self):
        return test_iteration30.Iteration30ExecutorBindingAuthorizationPackageReadinessAuthorizationDecisionTest(
            methodName="test_current_repository_stays_blocked_deterministic_and_zero_rework"
        )

    def fixture(self, name):
        return Path(__file__).resolve().parents[1] / "fixtures" / "iteration31" / name

    def make_executor_binding_authorization_package_readiness_authorization_decision(
        self, root, date=DATE, mode="synthetic", qualified=False
    ):
        h = self.helper()
        _, engine = h.make_executor_binding_authorization_package_readiness_authorization_review(root, date, mode, qualified=qualified)
        kwargs = {}
        if qualified:
            kwargs["integration_execution_executor_binding_authorization_package_readiness_authorization_decision_fixture_root"] = (
                h.write_fixture(Path(root) / "i30-qualified", engine)
            )
    ...
    def locked(self, engine):
        ...
        values["production-integration-execution-executor-binding-authorization-package-readiness-authorization-decision"] = (
    ...
    def write_fixture(
        ...
        *,
        ...
        src = self.fixture("synthetic-executor-binding-authorization-package-readiness-authorization-package-complete")
        policy = json.loads((src / "executor-binding-authorization-package-readiness-authorization-package-policy.json").read_text())
        manifest = json.loads((src / "executor-binding-authorization-package-readiness-authorization-package-manifest.json").read_text())
        record = json.loads((src / "executor-binding-authorization-package-readiness-authorization-package-record.json").read_text())

        gate = ProductionIntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationPackage(
        ...
        manifest["executor_binding_authorization_package_readiness_authorization_decision_binding"] = {
        ...
        record["executor_binding_authorization_package_readiness_authorization_decision_evidence_ids"] = bound["executor_binding_authorization_package_readiness_authorization_decision_evidence_ids"]
        record["executor_binding_authorization_package_readiness_authorization_decision_evidence_digests"] = bound["executor_binding_authorization_package_readiness_authorization_decision_evidence_digests"]
        record.update(
            {
                "executor_binding_authorization_package_readiness_authorization_decision_artifact_digest": bound[
                    "executor_binding_authorization_package_readiness_authorization_decision_artifact_digest"
                ],
                "executor_binding_authorization_package_readiness_authorization_decision_id": bound[
        ...
        body.pop("authorization_package_record_id", None)
        record["authorization_package_record_id"] = digest(body)

        (root / "executor-binding-authorization-package-readiness-authorization-package-policy.json").write_text(json.dumps(policy))
        (root / "executor-binding-authorization-package-readiness-authorization-package-manifest.json").write_text(json.dumps(manifest))
        if include_record:
            (root / "executor-binding-authorization-package-readiness-authorization-package-record.json").write_text(
                json.dumps(record)
            )
        return root
    ...
    def test_current_repository_stays_blocked_deterministic_and_zero_rework(self):
        with tempfile.TemporaryDirectory() as td:
            counts, engine = self.make_executor_binding_authorization_package_readiness_authorization_decision(td)
            ...
            self.assertEqual(data1["executor_binding_authorization_package_readiness_authorization_package_id"], data2["executor_binding_authorization_package_readiness_authorization_package_id"])
    ...
    def test_complete_review_requires_separate_record_and_complete_decision_evidence(self):
        with tempfile.TemporaryDirectory() as base:
            counts, engine = self.make_executor_binding_authorization_package_readiness_authorization_decision(base, qualified=True)
            locked = self.locked(engine)
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                start_daily_brief(
                    self.DATE,
                    state_root=td,
                    integration_execution_executor_binding_authorization_package_readiness_authorization_package_fixture_root=fixture,
                    integration_execution_executor_binding_authorization_package_readiness_authorization_package_only=True,
                )
                data = self.data(td)
                ...
                self.assertEqual(data["executor_binding_authorization_package_readiness_authorization_package_evidence_ids"], [])
                self.safe(data)
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                data = self.data(td)
                self.assertEqual(data["classification"], "executor_binding_authorization_package_readiness_authorization_package_complete")
                self.assertEqual(
                ...
                self.assertEqual(
                    data["executor_binding_authorization_package_readiness_authorization_decision_classification"],
                ...
                schema = json.loads((Path(__file__).resolve().parents[1] / "schemas" /
                    "production-integration-execution-executor-binding-authorization-package-readiness-authorization-package.schema.json").read_text())
                for field in schema["required"] + schema["allOf"][0]["then"]["required"]:
                    self.assertIn(field, data)
                self.assertEqual(len(data["executor_binding_authorization_package_readiness_authorization_package_evidence_ids"]), 10)
                self.assertEqual(len(set(data["executor_binding_authorization_package_readiness_authorization_package_evidence_ids"])), 10)
                for position in range(1, 11):
                    receipt = e.store.read_json(
                        e.store.run_dir
                        / f"executor-binding-authorization-package-readiness-authorization-package-evidence-{position:02d}.json"
                    )
    ...
    def test_fail_closed_iteration29_identity_and_provenance_substitution(self):
        with tempfile.TemporaryDirectory() as base:
            self.make_executor_binding_authorization_package_readiness_authorization_decision(base, qualified=True)
            ...
            cases = [
                lambda d: d.__setitem__("executor_binding_authorization_package_readiness_authorization_decision_id", "sha256:" + "0" * 64),
                lambda d: d.__setitem__("executor_binding_authorization_package_readiness_authorization_decision_policy_id", "changed"),
                lambda d: d.__setitem__("executor_binding_authorization_package_readiness_authorization_decision_manifest_id", "changed"),
                lambda d: d.__setitem__("separate_executor_binding_authorization_package_readiness_authorization_decision_id", "sha256:" + "1" * 64),
                ...
                lambda d: d.__setitem__(
                    "executor_binding_authorization_package_readiness_authorization_decision_evidence_digests",
                    ["sha256:" + "3" * 64]
                    + d["executor_binding_authorization_package_readiness_authorization_decision_evidence_digests"][1:],
                ),
            ]
            for idx, fn in enumerate(cases):
                with self.subTest(case=idx), tempfile.TemporaryDirectory() as td:
                    ...
                    with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationPackageError):
                        start_daily_brief(
                            self.DATE,
                            state_root=td,
                            integration_execution_executor_binding_authorization_package_readiness_authorization_package_fixture_root=(
            ...
            fixture_cases = {
                ...
                "policy-version": dict(
                    mutate_policy=lambda p: p.update(
                        {"executor_binding_authorization_package_readiness_authorization_package_policy_version": "2.0.0"}
                    )
                ),
    ...
    def test_fail_closed_reordered_duplicated_missing_substituted_corrupted_receipts(self):
        with tempfile.TemporaryDirectory() as base:
                ...
                with tempfile.TemporaryDirectory() as td:
                    ...
                    with self.assertRaises(
                        ...
                        start_daily_brief(
                            self.DATE,
                            state_root=td,
                            integration_execution_executor_binding_authorization_package_readiness_authorization_package_fixture_root=Path(td) / "fixture",
                            integration_execution_executor_binding_authorization_package_readiness_authorization_package_only=True,
                        )
            ...
            expect_receipt_failure(
                lambda e: mutate_final(
                    ...
                    lambda d: d.__setitem__(
                        ...
                        list(reversed(d["executor_binding_authorization_package_readiness_authorization_package_evidence_ids"])),
                    ),
                )
            )
            expect_receipt_failure(
                lambda e: mutate_final(
                    ...
                    lambda d: d.__setitem__(
                        "executor_binding_authorization_package_readiness_authorization_package_evidence_ids",
                        [d["executor_binding_authorization_package_readiness_authorization_package_evidence_ids"][0]] * 10,
                    ),
                )
            )
            expect_receipt_failure(
                lambda e: mutate_final(
                    ...
                    lambda d: d.__setitem__(
                        "executor_binding_authorization_package_readiness_authorization_package_evidence_ids",
                        d["executor_binding_authorization_package_readiness_authorization_package_evidence_ids"][:-1],
                    ),
                )
            )

            def corrupt_receipt(e):
                path = e.store.run_dir / "executor-binding-authorization-package-readiness-authorization-package-evidence-05.json"
                r = e.store.read_json(path)
                r["source_executor_binding_authorization_package_readiness_authorization_decision_evidence_id"] = "sha256:" + "9" * 64
            ...
            def missing_receipt(e):
                (e.store.run_dir / "executor-binding-authorization-package-readiness-authorization-package-evidence-05.json").unlink()
        ...
        with tempfile.TemporaryDirectory() as base:
            ...
            for case in ("reordered", "duplicated", "missing", "substituted", "corrupted", "version", "upstream-stale"):
                with self.subTest(case=case), tempfile.TemporaryDirectory() as td:
                    self.clone_root(base, td)
                    e = RunEngine(td, self.DATE)
                    p = e.store.run_dir / "executor-binding-authorization-package-readiness-authorization-decision-evidence-05.json"
                    q = e.store.run_dir / "executor-binding-authorization-package-readiness-authorization-decision-evidence-06.json"
                    if case == "missing":
                        p.unlink()
                    elif case == "reordered":
        ...
        with tempfile.TemporaryDirectory() as base:
            ...
            for case in ("cache", "artifact-version", "artifact-id", "record-id", "manifest-version"):
                with self.subTest(case=case), tempfile.TemporaryDirectory() as td:
                    ...
                    e = RunEngine(td, self.DATE)
                    if case == "cache":
                        p = e.store.run_dir / "executor-binding-authorization-package-readiness-authorization-package-state.json"
                        a = e.store.read_json(p)
                        a["evaluation"]["classification"] = "blocked"
                    elif case.startswith("artifact"):
                        p = e.store.artifact_path("production-integration-execution-executor-binding-authorization-package-readiness-authorization-package")
                        a = e.store.read_json(p)
                        k = "executor_binding_authorization_package_readiness_authorization_package_" + ("schema_version" if case.endswith("version") else "id")
                        a["data"][k] = "changed"
                        a["content_digest"] = semantic_digest(a)
                    elif case == "record-id":
                        p = Path(td) / "fixture" / "executor-binding-authorization-package-readiness-authorization-package-record.json"
                        a = e.store.read_json(p)
                        a["authorization_package_record_id"] = "changed"
                    else:
                        p = Path(td) / "fixture" / "executor-binding-authorization-package-readiness-authorization-package-manifest.json"
                        a = e.store.read_json(p)
                        a["executor_binding_authorization_package_readiness_authorization_package_manifest_version"] = "9.0.0"
                    e.store._atomic_write(p, a)
                    with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationPackageError):
                        start_daily_brief(self.DATE, state_root=td,
                            integration_execution_executor_binding_authorization_package_readiness_authorization_package_fixture_root=Path(td) / "fixture",
    ...
    def recovery(self, source_root, boundary):
        ...
        with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationPackageBoundaryFailure):
            failing.run()
        durable = {p.name: (p.read_bytes(), p.stat().st_mtime_ns)
                   for p in engine.store.run_dir.glob("executor-binding-authorization-package-readiness-authorization-package-evidence-*.json")}
        start_daily_brief(
            self.DATE,
            state_root=root,
            integration_execution_executor_binding_authorization_package_readiness_authorization_package_fixture_root=fixture,
        ...
        self.assertEqual(
            fresh.store.read_json(
                fresh.store.run_dir / "executor-binding-authorization-package-readiness-authorization-package-incident.json"
            )["result"],
            "recovered",
        )
        ...
        state = fresh.store.read_json(fresh.store.run_dir / "executor-binding-authorization-package-readiness-authorization-package-state.json")
    ...
    def test_targeted_recovery_fresh_engine_no_rework(self):
        with tempfile.TemporaryDirectory() as base:
            ...
            temp, engine = self.recovery(base, "executor_binding_authorization_package_readiness_authorization_package:evaluation")
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-authorization-package-readiness-authorization-package-state.json"
            )
            self.assertEqual(state["attempts"]["evaluation"], 2)
            temp.cleanup()

            temp, engine = self.recovery(base, "executor_binding_authorization_package_readiness_authorization_package:receipt:5")
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-authorization-package-readiness-authorization-package-state.json"
            )
            ...
            temp, engine = self.recovery(
                base, "executor_binding_authorization_package_readiness_authorization_package:final_executor_binding_authorization_package_readiness_authorization_package_artifact"
            )
            state = engine.store.read_json(
                engine.store.run_dir / "executor-binding-authorization-package-readiness-authorization-package-state.json"
            )
    ...
    def test_bounded_exclusive_production_fail_closed_and_schema(self):
        ...
        schema = json.loads(
            (
                ...
                / "schemas"
                / "production-integration-execution-executor-binding-authorization-package-readiness-authorization-package.schema.json"
            ).read_text()
        )
        for key in (
            "production_action_authorized",
    ...
    def test_three_run_exit_gate_is_blocked_stable_and_independent(self):
        ...
        for date in ("2026-09-22", "2026-09-23", "2026-09-24"):
            with tempfile.TemporaryDirectory() as td:
                counts, engine = self.make_executor_binding_authorization_package_readiness_authorization_decision(
                    td, date, "shadow"
                )
                locked = self.locked(engine)
                prior_files = self.snapshot(engine)
                ...
                self.assertEqual(data["classification"], "blocked")
                self.assertEqual(data["classification_reason_codes"][0], "ITERATION30_EXECUTOR_BINDING_AUTHORIZATION_PACKAGE_READINESS_AUTHORIZATION_DECISION_BLOCKED")
                artifact = engine.store.load_artifact("production-integration-execution-executor-binding-authorization-package-readiness-authorization-package")
                self.emit("exit_run", {"date": date, "mode": "shadow", "artifact_digest": artifact["content_digest"],
                          "decision_id": data["executor_binding_authorization_package_readiness_authorization_package_id"],
                          "source_digest": data["executor_binding_authorization_package_readiness_authorization_decision_artifact_digest"],
                ...
                outcomes.append(
                    (
                        ...
                        data["executor_binding_authorization_package_readiness_authorization_package_id"],
                    )
                )
    ...
    def test_decision_record_exact_inventory_and_all_bound_identities(self):
        with tempfile.TemporaryDirectory() as base:
            ...
            original = json.loads((fixture / "executor-binding-authorization-package-readiness-authorization-package-record.json").read_text())
            changes = {k: "sha256:" + "f" * 64 for k in original
                       if k.endswith(("_id", "_digest")) and k != "authorization_package_record_id"}
            ...
            for field in ("executor_binding_authorization_package_readiness_authorization_decision_evidence_ids",
                          "executor_binding_authorization_package_readiness_authorization_decision_evidence_digests"):
            ...
            for name, value in changes.items():
                with self.subTest(name=name), tempfile.TemporaryDirectory() as td:
                    self.clone_root(base, td)
                    path = Path(td) / "fixture" / "executor-binding-authorization-package-readiness-authorization-package-record.json"
                    record = deepcopy(original)
                    record[name.split(":")[0]] = value
                    record["authorization_package_record_id"] = digest({k:v for k,v in record.items() if k != "authorization_package_record_id"})
                    path.write_text(json.dumps(record))
                    with self.assertRaises(IntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationPackageError):
                        start_daily_brief(self.DATE, state_root=td,
                            integration_execution_executor_binding_authorization_package_readiness_authorization_package_only=True,
                            integration_execution_executor_binding_authorization_package_readiness_authorization_package_fixture_root=path.parent)
    ...
    def test_unsafe_unknown_fields_and_all_mutations_rejected_on_blocked_source(self):
        with tempfile.TemporaryDirectory() as base:
            self.make_executor_binding_authorization_package_readiness_authorization_decision(base)
    ...
    def test_all_bounded_modes_exclusive_and_prior_gate_never_prepared_or_built(self):
        ...
        from new_daily_ai_brief.integration_execution_executor_binding_authorization_package_readiness_authorization_decision import ProductionIntegrationExecutionExecutorBindingAuthorizationPackageReadinessAuthorizationDecision as Prior
        option = "integration_execution_executor_binding_authorization_package_readiness_authorization_package_only"
        ...
        with tempfile.TemporaryDirectory() as td:
            self.make_executor_binding_authorization_package_readiness_authorization_decision(td)
            with patch.object(Prior, "prepare", side_effect=AssertionError("prior evaluation forbidden")), patch.object(Prior, "build_executor_binding_authorization_package_readiness_authorization_decision", side_effect=AssertionError("prior rebuild forbidden")):
