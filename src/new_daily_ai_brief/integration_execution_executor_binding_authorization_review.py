...
from .contracts import (
    ...
    EXECUTION_EXECUTOR_BINDING_REHEARSAL_POLICY_VERSION,
    EXECUTION_EXECUTOR_BINDING_REHEARSAL_SCHEMA_VERSION,
    EXECUTION_EXECUTOR_BINDING_REHEARSAL_RECEIPT_VERSION,
    ...
    EXECUTION_EXECUTOR_BINDING_AUTHORIZATION_REVIEW_RECORD_VERSION,
...
class IntegrationExecutionExecutorBindingAuthorizationReviewError(ContractError):
    pass


class IntegrationExecutionExecutorBindingAuthorizationReviewBoundaryFailure(
...
class ProductionIntegrationExecutionExecutorBindingAuthorizationReview:
    """Deterministic synthetic-only Iteration 23 executor-binding authorization review gate."""

    ARTIFACT_TYPE = "production-integration-execution-executor-binding-authorization-review"
    SOURCE_ARTIFACT_TYPE = "production-integration-execution-executor-binding-rehearsal"
    ...
    POLICY_KEYS = {
        "schema_version",
        "executor_binding_authorization_review_policy_version",
        "policy_id",
        "synthetic_only",
    ...
    MANIFEST_KEYS = {
        "schema_version",
        "executor_binding_authorization_review_manifest_version",
        "manifest_id",
        "synthetic_only",
        "production_action_authorized",
        "production_cutover_authorized",
        ...
        "rollback_execution_authorized",
        "real_integration_steps_enabled",
        "executor_binding_rehearsal_binding",
        "evidence",
        *MUTATION_FLAGS,
    }
    EVIDENCE_KEYS = {
    ...
    RECORD_KEYS = {
        ...
        "scope",
        "synthetic_only",
        "executor_binding_rehearsal_artifact_digest",
        "executor_binding_rehearsal_id",
        "executor_binding_rehearsal_policy_id",
        "executor_binding_rehearsal_policy_digest",
        "executor_binding_rehearsal_manifest_id",
        "executor_binding_rehearsal_manifest_digest",
        "separate_executor_binding_rehearsal_id",
        "separate_executor_binding_rehearsal_digest",
        "synthetic_binding_plan_descriptor_id",
        "synthetic_binding_plan_descriptor_digest",
        "executor_binding_rehearsal_receipt_set_digest",
    ...
    def __init__(
        ...
        *,
        ...
        failure_class: str = "synthetic_integration_execution_executor_binding_authorization_review_boundary_failure",
    ):
        self.store = store
        ...
        self.failure_boundary_id = (
            failure_boundary_id.removeprefix("executor_binding_authorization_review:")
            if failure_boundary_id
            else None
        )
        self.failure_class = failure_class
        self._failure_fired = False
        self.state_path = self.store.run_dir / "executor-binding-authorization-review-state.json"
        self.metrics_path = self.store.run_dir / "executor-binding-authorization-review-metrics.json"
        self.incident_path = self.store.run_dir / "executor-binding-authorization-review-incident.json"
    ...
    def _read_json_fixture(self, name: str, *, optional: bool = False) -> dict[str, Any] | None:
        if self.mode == "production" or self.fixture_root is None:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
        ...
        if not path.exists():
            if optional:
                return None
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                f"executor-binding-authorization-review fixture missing: {name}"
            )
    ...
    def _source_validation_path(self, position: int) -> Path:
        return self.store.run_dir / f"executor-binding-rehearsal-receipt-{position:02d}.json"

    def _load_source_validations(self, data: dict[str, Any]) -> list[dict[str, Any]]:
        ids = list(data.get("executor_binding_rehearsal_receipt_ids") or [])
        digests = list(data.get("executor_binding_rehearsal_receipt_digests") or [])
        if len(ids) != len(digests):
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "Iteration 22 executor-binding-rehearsal provenance inventory length mismatch"
            )
        if len(ids) != len(set(ids)) or len(digests) != len(set(digests)):
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "Iteration 22 executor-binding-rehearsal provenance inventory contains duplicates"
            )
        classification = data.get("classification")
        if classification == "executor_binding_rehearsal_complete" and len(ids) != 10:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "executor_binding_rehearsal_complete requires ten Iteration 22 provenance validations"
            )
        if classification != "executor_binding_rehearsal_complete" and ids:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "non-qualified Iteration 22 input cannot carry provenance validations"
            )
        ...
        for position, (expected_id, expected_digest) in enumerate(zip(ids, digests), start=1):
            ...
            if record is None:
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    f"Iteration 22 provenance validation missing at position {position}"
                )
            if record.get("receipt_id") != expected_id or digest(record) != expected_digest:
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    f"Iteration 22 provenance validation changed at position {position}"
                )
            if (
                ...
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    f"Iteration 22 provenance validation unsafe at position {position}"
                )
            body = {key: value for key, value in record.items() if key != "receipt_id"}
            if (record.get("receipt_id") != digest(body)
                    or record.get("schema_version") != EXECUTION_EXECUTOR_BINDING_REHEARSAL_SCHEMA_VERSION
                    or record.get("receipt_version") != EXECUTION_EXECUTOR_BINDING_REHEARSAL_RECEIPT_VERSION
                    or record.get("executor_binding_rehearsal_record_id") != data.get("separate_executor_binding_rehearsal_id")
                ...
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    f"Iteration 22 receipt identity or safety mismatch at position {position}"
                )
            result.append(record)
        if digest(result) != data.get("executor_binding_rehearsal_receipt_set_digest"):
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError("Iteration 22 receipt set mismatch")
        return result
    ...
    def _bound_upstream_identity(
        ...
        return {
            **deepcopy(semantic_identity),
            "executor_binding_rehearsal_artifact_digest": artifact["content_digest"],
            "executor_binding_rehearsal_id": data["executor_binding_rehearsal_id"],
            "executor_binding_rehearsal_classification": data["classification"],
            "executor_binding_rehearsal_reason_codes": deepcopy(
                data.get("classification_reason_codes") or []
            ),
            "executor_binding_rehearsal_policy_id": data.get(
                "executor_binding_rehearsal_policy_id"
            ),
            "executor_binding_rehearsal_policy_digest": data.get(
                "executor_binding_rehearsal_policy_digest"
            ),
            "executor_binding_rehearsal_manifest_id": data.get(
                "executor_binding_rehearsal_manifest_id"
            ),
            "executor_binding_rehearsal_manifest_digest": data.get(
                "executor_binding_rehearsal_manifest_digest"
            ),
            "separate_executor_binding_rehearsal_id": data.get(
                "separate_executor_binding_rehearsal_id"
            ),
            "separate_executor_binding_rehearsal_digest": data.get(
                "separate_executor_binding_rehearsal_digest"
            ),
            ...
            "executor_binding_rehearsal_receipt_ids": deepcopy(
                data.get("executor_binding_rehearsal_receipt_ids") or []
            ),
            "executor_binding_rehearsal_receipt_digests": deepcopy(
                data.get("executor_binding_rehearsal_receipt_digests") or []
            ),
            "executor_binding_rehearsal_receipt_set_digest": digest(validations),
            "canonical_chain_digest": data.get("canonical_chain_digest"),
            "iteration22_semantic_identity": semantic_identity,
            "iteration22_semantic_identity_digest": digest(semantic_identity),
        }
    ...
    def _context(self, run: dict[str, Any]) -> dict[str, Any]:
        ...
        if data.get("executor_binding_rehearsal_schema_version") != (
            EXECUTION_EXECUTOR_BINDING_REHEARSAL_SCHEMA_VERSION
        ):
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "unsupported Iteration 22 executor-binding-rehearsal schema version"
            )
        if data.get("executor_binding_rehearsal_policy_version") != (
            EXECUTION_EXECUTOR_BINDING_REHEARSAL_POLICY_VERSION
        ):
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "unsupported Iteration 22 executor-binding-rehearsal policy version"
            )
        if data.get("executor_binding_rehearsal_id") != digest(self._source_identity(data)):
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "Iteration 22 executor_binding_rehearsal_id does not match semantic identity"
            )
        self._assert_no_authority(data, "Iteration 22 executor-binding-rehearsal artifact")
        for flag in (
            "real_executor_bound",
            "real_executor_invoked",
            "production_credentials_present",
            ...
            if data.get(flag) is not False:
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    f"Iteration 22 executor-binding-rehearsal artifact must keep {flag}=false"
                )
        if data.get("synthetic_only") is not True:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "Iteration 22 executor-binding-rehearsal artifact must be synthetic-only"
            )
        ...
        if descriptor is not None:
            if (
                ...
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    "Iteration 22 synthetic binding plan is live, invocable, credentialed, targeted, paid, or mutating"
                )
        if data.get("executor_binding_rehearsal_policy_id") != (
            "production-integration-execution-executor-binding-rehearsal-v1"
        ):
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "Iteration 22 policy identity changed"
            )
        if data.get("executor_binding_rehearsal_manifest_id") != (
            "production-integration-execution-executor-binding-rehearsal-manifest-v1"
        ):
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "Iteration 22 manifest identity changed"
            )
        classification = data.get("classification")
        if classification not in {
            "blocked",
            "executor_binding_rehearsal_complete",
            "invalid",
        }:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "unsupported Iteration 22 classification"
            )
        if classification == "executor_binding_rehearsal_complete":
            if not data.get("separate_executor_binding_rehearsal_id"):
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    "qualified Iteration 22 input requires separate preflight identity"
                )
            if not data.get("synthetic_binding_plan_descriptor_id"):
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    "qualified Iteration 22 input requires binding-plan identity"
                )
        state = self.store.read_json(
            self.store.run_dir / "executor-binding-rehearsal-state.json"
        )
        ...
        if (
            ...
            or evaluation.get("executor_binding_rehearsal_policy_id")
            != data.get("executor_binding_rehearsal_policy_id")
            or evaluation.get("executor_binding_rehearsal_manifest_id")
            != data.get("executor_binding_rehearsal_manifest_id")
            or input_identity.get("executor_binding_rehearsal_policy_digest")
            != data.get("executor_binding_rehearsal_policy_digest")
            or input_identity.get("executor_binding_rehearsal_manifest_digest")
            != data.get("executor_binding_rehearsal_manifest_digest")
            or preflight_binding.get("rehearsal_record_id")
            != data.get("separate_executor_binding_rehearsal_id")
            or preflight_binding.get("rehearsal_record_digest")
            != data.get("separate_executor_binding_rehearsal_digest")
            ...
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "Iteration 22 durable executor-binding-rehearsal provenance does not match locked artifact"
            )
        ...
        if (digest(bound_source) != data.get("bound_upstream_identity_digest")
            ...
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError("Iteration 22 upstream identity changed")
        validations = self._load_source_validations(data)
        bound = self._bound_upstream_identity(artifact, data, validations)
        return {
            "executor_binding_rehearsal_artifact": artifact,
            "executor_binding_rehearsal_data": data,
            "source_executor_binding_rehearsal_receipts": validations,
    ...
    def _validate_policy_manifest(
        ...
        if set(policy) != self.POLICY_KEYS:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "Iteration 23 policy contains unsupported fields"
            )
        if set(manifest) != self.MANIFEST_KEYS:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "Iteration 23 manifest contains unsupported fields"
            )
        if (
            policy.get("schema_version")
            != EXECUTION_EXECUTOR_BINDING_AUTHORIZATION_REVIEW_SCHEMA_VERSION
            or policy.get("executor_binding_authorization_review_policy_version")
            != EXECUTION_EXECUTOR_BINDING_AUTHORIZATION_REVIEW_POLICY_VERSION
            or policy.get("policy_id")
            != "production-integration-execution-executor-binding-authorization-review-v1"
            or policy.get("synthetic_only") is not True
            or policy.get("allowed_upstream_classification")
            != "executor_binding_rehearsal_complete"
            or policy.get("classifications")
            != ["blocked", "executor_binding_authorization_review_complete", "invalid"]
            ...
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "unsupported or unsafe Iteration 23 executor-binding-authorization-review policy"
            )
        if (
            manifest.get("schema_version")
            != EXECUTION_EXECUTOR_BINDING_AUTHORIZATION_REVIEW_SCHEMA_VERSION
            or manifest.get("executor_binding_authorization_review_manifest_version") != "1.0.0"
            or manifest.get("manifest_id")
            != "production-integration-execution-executor-binding-authorization-review-manifest-v1"
            or manifest.get("synthetic_only") is not True
        ):
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "unsupported Iteration 23 executor-binding-authorization-review manifest"
            )
        self._assert_no_authority(manifest, "Iteration 23 manifest")
        evidence = manifest.get("evidence") or {}
        if set(evidence) != self.EVIDENCE_KEYS:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "Iteration 23 manifest evidence contains unsupported fields"
            )
        ...
        if set(evidence.get("stop_abort_conditions") or []) != required:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "executor-binding-authorization-review stop/abort conditions incomplete"
            )
    ...
    def _record_binding(
        ...
        if set(record) != self.RECORD_KEYS:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "executor-binding-authorization-review record contains unsupported fields"
            )
        body = deepcopy(record)
        supplied = body.pop("authorization_review_record_id")
        if supplied != digest(body):
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "executor-binding-authorization-review record semantic identity changed"
            )
        bound = context["bound_upstream_identity"]
        expected = {
            "schema_version": EXECUTION_EXECUTOR_BINDING_AUTHORIZATION_REVIEW_SCHEMA_VERSION,
            "record_version": EXECUTION_EXECUTOR_BINDING_AUTHORIZATION_REVIEW_RECORD_VERSION,
            "record_kind": "production-integration-execution-executor-binding-authorization-review-record",
            "decision": "complete_synthetic_executor_binding_authorization_review",
            "approved": True,
            "scope": "execution_executor_binding_authorization_review_only",
            "synthetic_only": True,
            "executor_binding_rehearsal_artifact_digest": bound[
                "executor_binding_rehearsal_artifact_digest"
            ],
            "executor_binding_rehearsal_id": bound["executor_binding_rehearsal_id"],
            "executor_binding_rehearsal_policy_id": bound[
                "executor_binding_rehearsal_policy_id"
            ],
            "executor_binding_rehearsal_policy_digest": bound[
                "executor_binding_rehearsal_policy_digest"
            ],
            "executor_binding_rehearsal_manifest_id": bound[
                "executor_binding_rehearsal_manifest_id"
            ],
            "executor_binding_rehearsal_manifest_digest": bound[
                "executor_binding_rehearsal_manifest_digest"
            ],
            "separate_executor_binding_rehearsal_id": bound[
                "separate_executor_binding_rehearsal_id"
            ],
            "separate_executor_binding_rehearsal_digest": bound[
                "separate_executor_binding_rehearsal_digest"
            ],
            ...
            "executor_binding_rehearsal_receipt_set_digest": bound[
                "executor_binding_rehearsal_receipt_set_digest"
            ],
            "canonical_chain_digest": bound["canonical_chain_digest"],
            "bound_upstream_identity_digest": context["bound_upstream_identity_digest"],
            "executor_binding_authorization_review_manifest_id": manifest["manifest_id"],
        ...
        for key, value in expected.items():
            if record.get(key) != value:
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    f"executor-binding-authorization-review record binding changed: {key}"
                )
        return {
    ...
    def _policy_manifest_record(
        ...
        policy = self._read_json_fixture("executor-binding-authorization-review-policy.json")
        manifest = self._read_json_fixture("executor-binding-authorization-review-manifest.json")
        record = self._read_json_fixture(
            "executor-binding-authorization-review-record.json", optional=True
        )
    ...
    def _evaluate(
        ...
        source = context["bound_upstream_identity"][
            "executor_binding_rehearsal_classification"
        ]
        reasons = context["bound_upstream_identity"][
            "executor_binding_rehearsal_reason_codes"
        ]
        if source == "invalid":
            return {
        ...
        expected_source_binding = {
            "binding_mode": "exact_locked_executor_binding_rehearsal_artifact",
            "executor_binding_rehearsal_artifact_digest": bound[
                "executor_binding_rehearsal_artifact_digest"
            ],
            "executor_binding_rehearsal_id": bound["executor_binding_rehearsal_id"],
            ...
            "executor_binding_rehearsal_receipt_ids": bound[
                "executor_binding_rehearsal_receipt_ids"
            ],
            "executor_binding_rehearsal_receipt_digests": bound[
                "executor_binding_rehearsal_receipt_digests"
            ],
            "executor_binding_rehearsal_receipt_set_digest": bound[
                "executor_binding_rehearsal_receipt_set_digest"
            ],
            "canonical_chain_digest": bound["canonical_chain_digest"],
            "iteration22_semantic_identity_digest": bound[
                "iteration22_semantic_identity_digest"
            ],
            "bound_upstream_identity_digest": context["bound_upstream_identity_digest"],
        }
        if manifest.get("executor_binding_rehearsal_binding") != expected_source_binding:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "executor-binding-authorization-review manifest binds a different Iteration 22 identity"
            )
        ...
        return {
            "classification": "executor_binding_authorization_review_complete",
            "classification_reason_codes": [
    ...
    def _input_identity(
        ...
        return {
            ...
            "executor_binding_rehearsal_id": bound["executor_binding_rehearsal_id"],
            "bound_upstream_identity_digest": context["bound_upstream_identity_digest"],
            "executor_binding_authorization_review_policy_digest": digest(policy),
            "executor_binding_authorization_review_manifest_digest": digest(manifest),
    ...
    def _load_state(self, identity: dict[str, Any]) -> dict[str, Any]:
        ...
        if (
            ...
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "executor-binding-authorization-review durable state binds different inputs"
            )
        return state
    ...
    def _recover_incident_if_needed(self, state: dict[str, Any]) -> None:
        ...
        if incident and incident.get("result") == "pending":
            if incident.get("input_identity") != state.get("input_identity"):
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    "executor-binding-authorization-review incident binds different inputs"
                )
            incident["result"] = "recovered"
            self.store._atomic_write(self.incident_path, incident)

    def _receipt_path(self, position: int) -> Path:
        return self.store.run_dir / f"executor-binding-authorization-review-evidence-{position:02d}.json"
    ...
    def _receipt_record(
        ...
        body = {
            "schema_version": EXECUTION_EXECUTOR_BINDING_AUTHORIZATION_REVIEW_SCHEMA_VERSION,
            "receipt_version": EXECUTION_EXECUTOR_BINDING_AUTHORIZATION_REVIEW_EVIDENCE_VERSION,
            "executor_binding_rehearsal_id": bound["executor_binding_rehearsal_id"],
            "executor_binding_rehearsal_artifact_digest": bound[
                "executor_binding_rehearsal_artifact_digest"
            ],
            "executor_binding_authorization_review_record_id": evaluated[
                "rehearsal_binding"
            ]["authorization_review_record_id"],
            "synthetic_binding_plan_descriptor_id": bound[
                "synthetic_binding_plan_descriptor_id"
            ...
            "position": position,
            "source_executor_binding_rehearsal_receipt_id": source["receipt_id"],
            "source_executor_binding_rehearsal_receipt_digest": digest(source),
            "noop_verified": True,
    ...
    def _ensure_receipts(
        ...
        if evaluated["classification"] != "executor_binding_authorization_review_complete":
            return []
        sources = context["source_executor_binding_rehearsal_receipts"]
        if len(sources) != 10:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
        ...
        for position, source in enumerate(sources, start=1):
            ...
            if existing is not None:
                if existing != expected:
                    raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                        f"cached executor-binding-authorization-review evidence changed at position {position}"
                    )
            ...
            if self.failure_boundary_id == boundary and not self._failure_fired:
                ...
                raise IntegrationExecutionExecutorBindingAuthorizationReviewBoundaryFailure(
                    "executor_binding_authorization_review_evidence",
                    boundary,
                    self.failure_class,
                )
    ...
    def prepare(self, run: dict[str, Any]) -> dict[str, Any]:
        ...
        if state.get("evaluation") is None:
            ...
            if self.failure_boundary_id == "evaluation" and not self._failure_fired:
                ...
                self._write_metrics(state)
                raise IntegrationExecutionExecutorBindingAuthorizationReviewBoundaryFailure(
                    "executor_binding_authorization_review_evaluation",
                    "evaluation",
                    self.failure_class,
                )
            ...
            state["evaluation"] = {
                **outcome,
                ...
                "executor_binding_authorization_review_policy_id": policy["policy_id"],
                "executor_binding_authorization_review_policy_version": policy[
                    "executor_binding_authorization_review_policy_version"
                ],
                "executor_binding_authorization_review_manifest_id": manifest["manifest_id"],
            ...
            if digest(state["evaluation"]) != state.get("evaluation_digest"):
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError("cached review evaluation corrupted")
        ...
        evaluated["executor_binding_authorization_review_evidence_records"] = receipts
    ...
    def _artifact_data(self, evaluated: dict[str, Any]) -> dict[str, Any]:
        ...
        receipts = evaluated["executor_binding_authorization_review_evidence_records"]
        evidence = evaluated["evidence"]
        if evaluated["classification"] == "executor_binding_authorization_review_complete":
            if len(receipts) != 10:
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    "executor_binding_authorization_review_complete requires ten no-op receipts"
                )
            if len({x["receipt_id"] for x in receipts}) != 10:
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    "executor-binding-authorization-review evidence identities must be unique"
                )
        elif receipts:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
        ...
        identity = {
            "executor_binding_authorization_review_schema_version": (
                EXECUTION_EXECUTOR_BINDING_AUTHORIZATION_REVIEW_SCHEMA_VERSION
            ),
            "executor_binding_authorization_review_policy_version": evaluated[
                "executor_binding_authorization_review_policy_version"
            ],
            "executor_binding_authorization_review_policy_id": evaluated[
                "executor_binding_authorization_review_policy_id"
            ],
            "executor_binding_authorization_review_policy_digest": evaluated[
                "input_identity"
            ]["executor_binding_authorization_review_policy_digest"],
            "executor_binding_authorization_review_manifest_id": evaluated[
                "executor_binding_authorization_review_manifest_id"
            ],
            "executor_binding_authorization_review_manifest_digest": evaluated[
                "input_identity"
            ]["executor_binding_authorization_review_manifest_digest"],
            "separate_executor_binding_authorization_review_id": rehearsal_binding[
                "authorization_review_record_id"
            ],
            "separate_executor_binding_authorization_review_digest": rehearsal_binding[
            ...
            **deepcopy(evaluated["bound_upstream_identity"]),
            ...
            "executor_binding_authorization_review_evidence_set_digest": digest(receipts),
        ...
        return {
            **identity,
            "executor_binding_authorization_review_id": digest(identity),
            "real_integration_steps_enabled": 0,
            "real_integration_steps_executed": 0,
            "final_state": "Complete",
            "final_status": "complete_locked",
            "completion_scope": (
                "synthetic_shadow_execution_executor_binding_authorization_review_only"
            ),
            "synthetic_only": True,
            "production_action_authorized": False,
            "production_cutover_authorized": False,
    ...
    def validate_existing(
        ...
        if (
            ...
            raise IntegrationExecutionExecutorBindingAuthorizationReviewBoundaryFailure(
                "executor_binding_authorization_review_validation",
                "executor_binding_authorization_review_artifact",
                "cached executor-binding-authorization-review artifact invalid",
            )
        if existing.get("data") != self._artifact_data(evaluated):
            raise IntegrationExecutionExecutorBindingAuthorizationReviewBoundaryFailure(
                "executor_binding_authorization_review_validation",
                "executor_binding_authorization_review_artifact",
                "cached executor-binding-authorization-review artifact binds different identities",
            )
    ...
    def build_executor_binding_authorization_review(
        ...
        self._save_state(state)
        if (
            self.failure_boundary_id == "final_executor_binding_authorization_review_artifact"
            and not self._failure_fired
        ):
            self._failure_fired = True
            self._record_incident(
                "final_executor_binding_authorization_review_artifact", state
            )
            self._write_metrics(state)
            raise IntegrationExecutionExecutorBindingAuthorizationReviewBoundaryFailure(
                "executor_binding_authorization_review_artifact_assembly",
                "final_executor_binding_authorization_review_artifact",
                self.failure_class,
            )
    ...
    def finalize(self, artifact: dict[str, Any]) -> None:
        if not artifact or artifact.get("status") != "locked":
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "final executor-binding-authorization-review artifact is not locked"
            )
        if semantic_digest(artifact) != artifact.get("content_digest"):
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "final executor-binding-authorization-review artifact digest invalid"
            )
        data = artifact.get("data") or {}
        self._assert_no_authority(data, "Iteration 23 artifact")
        for flag in (
            "real_executor_bound",
            "real_executor_invoked",
            "production_credentials_present",
        ...
        classification = data.get("classification")
        if classification == "executor_binding_authorization_review_complete":
            if (
                data.get("executor_binding_rehearsal_classification")
                != "executor_binding_rehearsal_complete"
            ):
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    "executor_binding_authorization_review_complete requires complete Iteration 22 rehearsal"
                )
            if not data.get("separate_executor_binding_authorization_review_id"):
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    "executor_binding_authorization_review_complete requires separate authorization-review record"
                )
            ids = list(data.get("executor_binding_authorization_review_evidence_ids") or [])
            digests = list(data.get("executor_binding_authorization_review_evidence_digests") or [])
            if len(ids) != 10 or len(digests) != 10:
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    "executor_binding_authorization_review_complete requires ten receipts"
                )
            if len(ids) != len(set(ids)) or len(digests) != len(set(digests)):
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    "executor-binding-authorization-review evidence inventory contains duplicates"
                )
            loaded = []
            for position, (expected_id, expected_digest) in enumerate(
                ...
                if receipt is None:
                    raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                        f"executor-binding-authorization-review evidence missing at position {position}"
                    )
                if (
                    ...
                    raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                        f"unsafe or corrupted executor-binding-authorization-review evidence at position {position}"
                    )
            ...
            if digest(loaded) != data.get(
                ...
                raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                    "executor-binding-authorization-review evidence set digest changed"
                )
        elif classification not in {"blocked", "invalid"}:
            raise IntegrationExecutionExecutorBindingAuthorizationReviewError(
                "unsupported executor-binding-authorization-review classification"
            )
