...
function renderCoverage(){if(functionCoverage)$('parity-metrics').insertAdjacentHTML('beforeend',metric('Deployed function coverage',functionCoverage.deployed_passed+'/69')+metric('Live acceptance coverage',functionCoverage.live_passed+'/69')+metric('Still partial',functionCoverage.partial??'Unavailable')+metric('Intentional exclusions preserved',functionCoverage.intentional_exclusions_preserved??'Unavailable'));}fetch('/coverage.json').then(r=>r.json()).then(d=>{functionCoverage=d;renderCoverage();}).catch(()=>{});
