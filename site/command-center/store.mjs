...
export async function lastSnapshot(db){const records=await rows(db.prepare('SELECT payload,verified_at FROM cc_snapshots ORDER BY verified_at DESC LIMIT 20'));for(const r of records){const state=JSON.parse(r.payload);if(state.schema_version==='2.0.0'&&state.source?.main_sha&&state.executive?.edition_date)return {snapshot:state,verified_at:r.verified_at};}return null;}
